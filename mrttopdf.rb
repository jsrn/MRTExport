...
require "mysql2"
require "base64"
require_relative "dbdb_builder.rb"
require_relative "data_band.rb"
require_relative "stylist.rb"
...
class MRTToPDF
  ...
  def usage
    puts "USAGE:"
    puts "./mrttopdf.rb <report file> <quote id>"
    exit
  end
  ...
  def render_image(node)
    ...
    path = File.dirname(File.readlink(__FILE__)) << "/reports/tmp"

    File.open(path,'w') { |file| file.puts data }

    image = path
    @pdf.image image, :at => [x, 0 + h], :width => w 
